...
def get_cast(info_box_html):
    ...
    if cast_raw:
        cast_cleaned = re.sub(r'<.*?>', '.', cast_raw[0])  # Usunięcie tagów HTML
        cast_list = cast_cleaned.split('.')
        # Filtrujemy wyniki, aby wyeliminować niepożądane wartości
...
# Tworzenie nowego pliku JSON i zapisywanie danych
output_filename = 'wikidata_films_updated.json'
with open(output_filename, 'w'):  # Tworzenie pustego pliku
    pass
